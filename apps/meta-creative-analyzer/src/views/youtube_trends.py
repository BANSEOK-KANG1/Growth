"""Marketing Keyword Gap → Shoot Brief view."""
...
def render() -> None:
    ...
    g4.metric("Category overlap", f"{gap['category_overlap_pct']:.0f}%")
    ...
    with st.expander("상세 데이터 — KR 트렌딩 baseline · 키워드 검색 결과 · 패턴"):
        ...
        with tab_kw:
            if keyword_df.empty:
                ...
                st.dataframe(
                    keyword_df[["title", "channel_title", "format_label", "title_hook", "views", "engagement_rate"]],
                    use_container_width=True,
                    hide_index=True,
                )
        with tab_pat:
            dim = st.radio(
                "차원", ["title_hook", "format_label", "category_name"], horizontal=True,
        ...
        csv = (
            keyword_df.to_csv(index=False).encode("utf-8-sig")
            if not keyword_df.empty
            else trending_df.to_csv(index=False).encode("utf-8-sig")
        )
