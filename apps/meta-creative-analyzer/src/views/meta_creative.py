"""Meta Creative Intelligence view."""
...
from bootstrap import is_streamlit_cloud
from meta_client import MetaAPIError, MetaClient, load_sample_dataframe
from recommend import CTA_LABELS, build_direction_brief
from transform import enrich_dataframe, overview_metrics
...
def render() -> None:
    ...
    if is_streamlit_cloud():
        st.sidebar.info("Sample mode 기본 · Meta API는 Secrets 설정 후 토글 ON")
    ...
    with tab_explorer:
        ...
        display_cols = [
            "ad_name", "format_tag", "hook_keyword", "call_to_action_type",
            "copy_preview", "spend", "ctr", "cpa", "cvr", "conversions",
        ]
        st.dataframe(
            filtered[display_cols].rename(
                columns={
                    "ad_name": "광고명", "format_tag": "포맷", "hook_keyword": "훅",
                    "call_to_action_type": "CTA", "copy_preview": "카피 미리보기",
                    "spend": "비용", "ctr": "CTR(%)", "cpa": "CPA",
                    "cvr": "CVR(%)", "conversions": "전환",
                }
            ),
    ...
    with tab_pattern:
        ...
        dim = st.radio(
            "분석 차원", ["format_tag", "hook_keyword", "call_to_action_type"], horizontal=True
        )
        stats = aggregate_by_dimension(df, dim)
        if not stats.empty:
            st.bar_chart(stats.set_index(dim)[["avg_cpa"]])
